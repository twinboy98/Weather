...
import type { Recommendation, WeatherBundle } from "@/lib/domain";
...
function cells(bundle: WeatherBundle | undefined): RainCell[] {
  ...
  return bundle.points
    ...
    .map((point) => ({
      ...
      amount: point.precipitationAmountMm ?? (point.precipitationRateMmh !== undefined ? point.precipitationRateMmh : null),
    }));
}

function rainRisk(cell: RainCell | undefined): number {
  if (!cell) return 0;
  const probability = cell.probability ?? (cell.amount && cell.amount > 0 ? 0.55 : 0);
...
function nextRainMessage(home: RainCell[], work: RainCell[]): string {
  ...
  const probability = candidate.probability === null ? "강수 신호" : `강수확률 ${Math.round(candidate.probability * 100)}%`;
  return `${kstHour(candidate.time)}부터 ${probability}가 있습니다.`;
}

export function RainWindowPanel({ home, work, outbound, inbound }: RainWindowPanelProps) {
  ...
  const routeCells = timeline.map((item, index) => ({
    time: item.time,
    probability: average(homeCells[index]?.probability, workCells[index]?.probability),
                ...
                <div className="mt-1.5 grid grid-cols-[3.75rem_repeat(12,minmax(0,1fr))] gap-1" key={label}>
                  ...
                  {timeline.map((timelineCell, index) => {
                    ...
                    return (
                      ...
                      >
                        {isRecommended && <span className="absolute inset-x-0 top-1 text-center text-xs" aria-label="추천 출발 시간">✦</span>}
                      </div>
                    );
                  })}
                </div>
...
function average(first: number | null | undefined, second: number | null | undefined): number | null {
  const values = [first, second].filter((value): value is number => value !== null && value !== undefined && Number.isFinite(value));
